...
def read_bin(bin_file):
    try:
        ...
        for i in range(len(contents) // 16):
            print(
                "0x%08x: 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x" % (
        ...
        if len(contents) % 16 != 0:
            ...
            for i in range(16):
                if (len(contents) // 16) * 16 + i < len(contents):
                    temp[i] = "%02x" % contents[(len(contents) // 16) * 16 + i]
            print(
                "0x%08x: 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s" % (
        ...
        print("Checksum (0x%08x - 0x%08x): 0x%08x, %d bytes" % (
        ...
        try:
            ...
            for num in range(num_128):
                ...
                while True:
                    if ser.in_waiting > 0:
                        ...
                        if data_rx == b'\r':
                            ...
                            if response == b't031179\r':
                                # print("Write Memory 0x%08x - 0x%08x Request Done (ACK), %04d / %04d" % (
                                ...
                                # print("Write Memory 0x%08x - 0x%08x Request Failed (NACK), %04d / %04d" % (
                                # addr, addr + 128 - 1, num + 1, num_128))
                                ...
                                # print("Write Memory 0x%08x - 0x%08x Request Failed (Unknown), %04d / %04d" % (
                                # addr, addr + 128 - 1, num + 1, num_128))
                    ...
                    if time.time() >= prev_time + 1.0:
                        print("Recv: %s" % response[0:].decode("utf-8"))
                        print("Write Memory 0x%08x - 0x%08x Request Failed (Timeout), %04d / %04d" % (
                        addr, addr + 128 - 1, num + 1, num_128))
                ...
                for i in range(16):
                    ...
                    while True:
                        if ser.in_waiting > 0:
                            ...
                            if data_rx == b'\r':
                                ...
                                if response == b't031179\r':
                                    ...
                                    # print("Write Memory 0x%08x - 0x%08x Failed (NACK), %04d / %04d" % (
                                    ...
                                    # print("Write Memory 0x%08x - 0x%08x Failed (Unknown), %04d / %04d" % (
                                    # addr + (i * 8), addr + ((i + 1) * 8) - 1, num + 1, num_128))
                        ...
                        if time.time() >= prev_time + 1.0:
                            ...
                            print("Write Memory 0x%08x - 0x%08x Failed (Timeout), %04d / %04d" % (
                            addr + (i * 8), addr + ((i + 1) * 8) - 1, num + 1, num_128))
                            break
                ...
                while True:
                    if ser.in_waiting > 0:
                        ...
                        if data_rx == b'\r':
                            # print("Recv: %s\\r" % response[0:-1].decode("utf-8"))
                            if response == b't031179\r':
                                # print("Write Memory 0x%08x - 0x%08x Response Done (ACK), %04d / %04d" % (
                                # addr, addr + 128 - 1, num + 1, num_128))
                                ...
                                # print("Write Memory 0x%08x - 0x%08x Response Failed (NACK), %04d / %04d" % (
                                # addr, addr + 128 - 1, num + 1, num_128))
                                ...
                                # print("Write Memory 0x%08x - 0x%08x Response Failed (Unknown), %04d / %04d" % (
                                # addr, addr + 128 - 1, num + 1, num_128))
                    ...
                    if time.time() >= prev_time + 1.0:
                        print("Recv: %s" % response[0:].decode("utf-8"))
                        print("Write Memory 0x%08x - 0x%08x Response Failed (Timeout), %04d / %04d" % (
                        addr, addr + 128 - 1, num + 1, num_128))
    ...
    if check_ser == True:
        try:
            ...
            for num in range(num_128):
                ...
                while True:
                    if ser.in_waiting > 0:
                        ...
                        if data_rx == b'\r':
                            ...
                            if response == b't011179\r':
                                # print("Read Memory 0x%08x - 0x%08x Request Done (ACK), %04d / %04d" % (
                                # addr, addr + 128 - 1, num + 1, num_128))
                                ...
                                # print("Read Memory 0x%08x - 0x%08x Request Failed (NACK), %04d / %04d" % (
                                # addr, addr + 128 - 1, num + 1, num_128))
                                ...
                                # print("Read Memory 0x%08x - 0x%08x Request Failed (Unknown), %04d / %04d" % (
                                # addr, addr + 128 - 1, num + 1, num_128))
                    ...
                    if time.time() >= prev_time + 1.0:
                        ...
                        # print("Read Memory 0x%08x - 0x%08x Request Failed (Timeout), %04d / %04d" % (
                        # addr, addr + 128 - 1, num + 1, num_128))
                        break
                ...
                while True:
                        ...
                        if data_rx == b'\r':
                            ...
                            if response[0:5] == b't0118':
                                if len(response) == 22:
                                    ...
                                    # print("Read Memory 0x%08x - 0x%08x Failed (Missing data), %04d / %04d" % (
                                    # addr + (i * 8), addr + ((i + 1) * 8) - 1, num + 1, num_128))
                            else:
                                for_break = True;
                                check_get_checksum = False

                                # print("Read Memory 0x%08x - 0x%08x Failed (Unknown), %04d / %04d" % (
                                # addr + (i * 8), addr + ((i + 1) * 8) - 1, num + 1, num_128))
                    ...
                    if time.time() >= prev_time + 1.0:
                        ...
                        print("Read Memory 0x%08x - 0x%08x Failed (Timeout), %04d / %04d" % (
                        addr, addr + 128 - 1, num + 1, num_128))
                        break
                ...
                while True:
                    if ser.in_waiting > 0:
                        ...
                        if data_rx == b'\r':
                            # print("Recv: %s\\r" % response[0:-1].decode("utf-8"))
                            if response == b't011179\r':
                                # print("Read Memory 0x%08x - 0x%08x Response Done (ACK), %04d / %04d" % (
                                # addr, addr + 128 - 1, num + 1, num_128))
                                ...
                                # print("Read Memory 0x%08x - 0x%08x Response Failed (NACK), %04d / %04d" % (
                                # addr, addr + 128 - 1, num + 1, num_128))
                                ...
                                # print("Read Memory 0x%08x - 0x%08x Response Failed (Unknown), %04d / %04d" % (
                                # addr, addr + 128 - 1, num + 1, num_128))
                    ...
                    if time.time() >= prev_time + 1.0:
                        ...
                        print("Read Memory 0x%08x - 0x%08x Response Failed (Timeout), %04d / %04d" % (
                        addr, addr + 128 - 1, num + 1, num_128))
                        break
            ...
            if check_get_checksum == True:
                print("Got checksum (0x%08x - 0x%08x): 0x%08x, %d bytes" % (
