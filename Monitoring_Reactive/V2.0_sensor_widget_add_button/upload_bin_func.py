...
form_window = uic.loadUiType("UI_V4.ui")[0]
print(form_window)
...
class UiMainWindow(QtWidgets.QMainWindow, form_window):
    ...
    def __init__(self):
        ...
        self.setupUi(self)
        self.bin_file = "../../V6.0_Reflect_Feedback/BIN2_F413ZH.bin"
        self.port = "11"
        ...
        self.gridlayout_sensors.addItem(self.verspacer,0,100)
        self.gridlayout_sensors.addItem(self.horspacer,100,0)
    ...
    def add_sensor_widget(self):
        if self.current_row * (self.max_column_count + 1) + self.current_column <= 24:
            # print(self.i)

            self.gridlayout_sensors.addWidget(self.sensor_widget_list[self.current_row * (self.max_column_count + 1) + self.current_column], self.current_row, self.current_column)
            time.sleep(0.02)

            self.sensor_widget_list[self.current_row * (self.max_column_count + 1) + self.current_column].sensor_name1_4.setText(str(self.current_row * (self.max_column_count + 1) + self.current_column))

            lst = ["hi","bye"] ### 무조건 텍스트임 조심
            self.sensor_widget_list[
                self.current_row * (self.max_column_count + 1) + self.current_column].combobox_sensor1_11.addItems(lst)

            self.sensor_widget_list[self.current_row * (
                        self.max_column_count + 1) + self.current_column].combobox_sensor1_11.setCurrentText(lst[random.randint(0,1)]) ## item에 없으면 안나오는 듯

            if self.current_column == self.max_column_count:
                self.current_column = 0
                self.current_row += 1
    ...
    def del_sensor_widget(self):
        if self.current_row * (self.max_column_count + 1) + self.current_column != 0:
            if self.current_column == 0:

                self.current_column = self.max_column_count
                self.current_row -= 1
            else:
                self.current_column -= 1

            self.gridlayout_sensors.removeWidget(self.sensor_widget_list[self.current_row * (self.max_column_count + 1) + self.current_column])
            sip.delete(self.sensor_widget_list[self.current_row * (self.max_column_count + 1) + self.current_column])
            self.sensor_widget_list[self.current_row * (self.max_column_count + 1) + self.current_column] = uic.loadUi("each_sensor.ui") # 초기화
