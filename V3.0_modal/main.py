...
# child_window = uic.loadUiType("UI_Child.ui")[0]
bin_file = ""
port = ""
skip_checksum = False

class Progressbar_loading(QThread):
    """
    ...
    def run(self):
        ...
        main_func(main_window.bin_file, main_window.port, main_window.skip_checksum)
...
class UiMainWindow(QtWidgets.QMainWindow, form_window):
    ...
    def Ok_Clicked(self):
        ...
        self.child = OptionWindow(self)

        QApplication.processEvents()

        thread = Progressbar_loading(self)
        thread.start()

        # check_main_func = main_func(self.bin_file, self.port, self.skip_checksum)
...
class OptionWindow(QtWidgets.QDialog):
    def __init__(self, main_window):  #부모 window 설정
        super(OptionWindow, self).__init__(main_window)
...
def read_bin(bin_file):
    print("Read_bin")
    main_window.child.count += 5
    main_window.child.progressBar.setValue(main_window.child.count)
    try:
        print("I'm in")
        ...
        #    print("0x%08x: n0x%02x%02x%02x%02x 0x%02x%02x%02x%02x 0x%02x%02x%02x%02x 0x%02x%02x%02x%02x" % (16*i, contents[16*i + 3], contents[16*i + 2], contents[16*i + 1], contents[16*i + 0], contents[16*i + 7], contents[16*i + 6], contents[16*i + 5], contents[16*i + 4], contents[16*i + 11], contents[16*i + 10], contents[16*i + 9], contents[16*i + 8], contents[16*i + 15], contents[16*i + 14], contents[16*i + 13], contents[16*i + 12]))
        #         # if len(contents) % 16 != 0:
        #         #    temp = ["  ", "  ", "  ", "  ", "  ", "  ", "  ", "  ", "  ", "  ", "  ", "  ", "  ", "  ", "  ", "  "]
        #         #    for i in range(16):
        #         #        if (len(contets) // 16) * 16 + i < len(contents):
        ...
        # for i in range(len(contents) // 16):
        #     print("hi2")
            # main_window.child.textEdit.append("0x%08x: 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x" % (
            #     16 * i, contents[16 * i + 0], contents[16 * i + 1], contents[16 * i + 2], contents[16 * i + 3],
            #     contents[16 * i + 4], contents[16 * i + 5], contents[16 * i + 6], contents[16 * i + 7],
            #     contents[16 * i + 8], contents[16 * i + 9], contents[16 * i + 10], contents[16 * i + 11],
            #     contents[16 * i + 12], contents[16 * i + 13], contents[16 * i + 14], contents[16 * i + 15]))
            # time.sleep(0.2)
            # QApplication.processEvents()
        ...
        if len(contents) % 16 != 0:
            ...
            for i in range(16):
                if (len(contents) // 16) * 16 + i < len(contents):
                    temp[i] = "%02x" % contents[(len(contents) // 16) * 16 + i]
            # main_window.child.textEdit.append("0x%08x: 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s" % (
            #     (len(contents) // 16) * 16, temp[0], temp[1], temp[2], temp[3], temp[4], temp[5], temp[6], temp[7],
        ...
        main_window.child.textEdit.append("Checksum (0x%08x - 0x%08x): 0x%08x, %d bytes" % (0x00000000, len(contents) - 1, crc, len(contents)))
...
def check_serial(check_ser, ser):
    ...
    if check_ser == True:
        try:
            ...
            main_window.child.textEdit.append("%s is alive" % port)
            ...
            main_window.child.textEdit.append("%s is dead" % port)
...
def check_hse_frequency(check_ser, ser):
    main_window.child.count += 5
    main_window.child.progressBar.setValue(main_window.child.count)
    print("check_hse_frequency")
    if check_ser == True:
        try:
            for i in range(16):
                ser.write(b't0048ffffffffffffffff\r')
                time.sleep(0.1)
    ...
    if check_ser == True:
        try:
                    ...
                    if data_rx == b'\r':
                        ...
                        if response[0:5] == b't0022':
                            if response[5:10] == b'0421\r':
                                main_window.child.textEdit.append("Get ID Done (ID: 0x%03s (STM32F446xx))" % response[6:9].decode("utf-8"))

                                chip_id = int(("0x%03s" % response[6:9].decode("utf-8")), 16)

                                break
                            else:
                                main_window.child.textEdit.append("Get ID Done (ID: 0x%03s)" % response[6:9].decode("utf-8"))
            ...
            if check_id == True:
                ...
                main_window.child.textEdit.append("Can not get ID")
...
def upload_bin(check_ser, ser, check_bin, contents, num_128):
    main_window.child.count += 25
    main_window.child.progressBar.setValue(main_window.child.count)

    main_window.child.textEdit.append("upload_bin")
        ...
        try:
            ...
            for num in range(num_128):
                ...
                while True:
                    if ser.in_waiting > 0:
                        ...
                        if data_rx == b'\r':
                            ...
                            if response == b't031179\r':
                                # main_window.child.textEdit.append("Write Memory 0x%08x - 0x%08x Request Done (ACK), %04d / %04d" % (
                                ...
                                # main_window.child.textEdit.append("Write Memory 0x%08x - 0x%08x Request Failed (NACK), %04d / %04d" % (
                                # addr, addr + 128 - 1, num + 1, num_128))
                                ...
                                # main_window.child.textEdit.append("Write Memory 0x%08x - 0x%08x Request Failed (Unknown), %04d / %04d" % (
                                # addr, addr + 128 - 1, num + 1, num_128))
                    ...
                    if time.time() >= prev_time + 1.0:
                        main_window.child.textEdit.append("Recv: %s" % response[0:].decode("utf-8"))
                        main_window.child.textEdit.append("Write Memory 0x%08x - 0x%08x Request Failed (Timeout), %04d / %04d" % (
                        addr, addr + 128 - 1, num + 1, num_128))
                    ...
                    while True:
                        if ser.in_waiting > 0:
                            ...
                            if data_rx == b'\r':
                                ...
                                if response == b't031179\r':
                                    # main_window.child.textEdit.append(
                                    #     "Write Memory 0x%08x - 0x%08x (0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s) Done (ACK), %04d / %04d" % (
                                    #     addr + (i * 8), addr + ((i + 1) * 8) - 1, request[5:7].decode("utf-8"),
                                    #     request[7:9].decode("utf-8"), request[9:11].decode("utf-8"),
                                    #     request[11:13].decode("utf-8"), request[13:15].decode("utf-8"),
                                    #     request[15:17].decode("utf-8"), request[17:19].decode("utf-8"),
                                    #     request[19:21].decode("utf-8"), num + 1, num_128))

                                    check_upload_bin = True

                                    break
                                elif response == b't03111f\r':
                                    # main_window.child.textEdit.append("Write Memory 0x%08x - 0x%08x Failed (NACK), %04d / %04d" % (
                                    # addr + (i * 8), addr + ((i + 1) * 8) - 1, num + 1, num_128))
                                    ...
                                    # main_window.child.textEdit.append("Write Memory 0x%08x - 0x%08x Failed (Unknown), %04d / %04d" % (
                                    # addr + (i * 8), addr + ((i + 1) * 8) - 1, num + 1, num_128))
                        ...
                        if time.time() >= prev_time + 1.0:
                            ...
                            main_window.child.textEdit.append("Write Memory 0x%08x - 0x%08x Failed (Timeout), %04d / %04d" % (
                            addr + (i * 8), addr + ((i + 1) * 8) - 1, num + 1, num_128))
                            break
                ...
                while True:
                    if ser.in_waiting > 0:
                        ...
                        if data_rx == b'\r':
                            # main_window.child.textEdit.append("Recv: %s\\r" % response[0:-1].decode("utf-8"))
                            if response == b't031179\r':
                                # main_window.child.textEdit.append("Write Memory 0x%08x - 0x%08x Response Done (ACK), %04d / %04d" % (
                                # addr, addr + 128 - 1, num + 1, num_128))
                                ...
                                # main_window.child.textEdit.append("Write Memory 0x%08x - 0x%08x Response Failed (NACK), %04d / %04d" % (
                                # addr, addr + 128 - 1, num + 1, num_128))
                                ...
                                # main_window.child.textEdit.append("Write Memory 0x%08x - 0x%08x Response Failed (Unknown), %04d / %04d" % (
                                # addr, addr + 128 - 1, num + 1, num_128))
                    ...
                    if time.time() >= prev_time + 1.0:
                        main_window.child.textEdit.append("Recv: %s" % response[0:].decode("utf-8"))
                        main_window.child.textEdit.append("Write Memory 0x%08x - 0x%08x Response Failed (Timeout), %04d / %04d" % (
                        addr, addr + 128 - 1, num + 1, num_128))
    ...
    if check_ser == True:
        try:
            ...
            for num in range(num_128):
                ...
                while True:
                    if ser.in_waiting > 0:
                        ...
                        if data_rx == b'\r':
                            ...
                            if response == b't011179\r':
                                # main_window.child.textEdit.append("Read Memory 0x%08x - 0x%08x Request Done (ACK), %04d / %04d" % (
                                # addr, addr + 128 - 1, num + 1, num_128))
                                ...
                                # main_window.child.textEdit.append("Read Memory 0x%08x - 0x%08x Request Failed (NACK), %04d / %04d" % (
                                # addr, addr + 128 - 1, num + 1, num_128))
                                ...
                                # main_window.child.textEdit.append("Read Memory 0x%08x - 0x%08x Request Failed (Unknown), %04d / %04d" % (
                                # addr, addr + 128 - 1, num + 1, num_128))
                    ...
                    if time.time() >= prev_time + 1.0:
                        ...
                        # main_window.child.textEdit.append("Read Memory 0x%08x - 0x%08x Request Failed (Timeout), %04d / %04d" % (
                        # addr, addr + 128 - 1, num + 1, num_128))
                        break
                ...
                while True:
                        ...
                        if data_rx == b'\r':
                            ...
                            if response[0:5] == b't0118':
                                if len(response) == 22:
                                    ...
                                    # main_window.child.textEdit.append(
                                    #     "Read Memory 0x%08x - 0x%08x (0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s) Done, %04d / %04d" % (
                                    #     addr + (i * 8), addr + ((i + 1) * 8) - 1, response[5:7].decode("utf-8"),
                                    #     response[7:9].decode("utf-8"), response[9:11].decode("utf-8"),
                                    #     response[11:13].decode("utf-8"), response[13:15].decode("utf-8"),
                                    #     response[15:17].decode("utf-8"), response[17:19].decode("utf-8"),
                                    #     response[19:21].decode("utf-8"), num + 1, num_128))
                                else:
                                    for_break = True;
                                    check_get_checksum = False

                                    # main_window.child.textEdit.append("Read Memory 0x%08x - 0x%08x Failed (Missing data), %04d / %04d" % (
                                    # addr + (i * 8), addr + ((i + 1) * 8) - 1, num + 1, num_128))
                            else:
                                for_break = True;
                                check_get_checksum = False

                                # main_window.child.textEdit.append("Read Memory 0x%08x - 0x%08x Failed (Unknown), %04d / %04d" % (
                                # addr + (i * 8), addr + ((i + 1) * 8) - 1, num + 1, num_128))
                    ...
                    if time.time() >= prev_time + 1.0:
                        ...
                        main_window.child.textEdit.append("Read Memory 0x%08x - 0x%08x Failed (Timeout), %04d / %04d" % (
                        addr, addr + 128 - 1, num + 1, num_128))
                        break
                ...
                while True:
                    ...
                    if ser.in_waiting > 0:
                        ...
                        if data_rx == b'\r':
                            # main_window.child.textEdit.append("Recv: %s\\r" % response[0:-1].decode("utf-8"))
                            if response == b't011179\r':
                                # main_window.child.textEdit.append("Read Memory 0x%08x - 0x%08x Response Done (ACK), %04d / %04d" % (
                                # addr, addr + 128 - 1, num + 1, num_128))
                                ...
                                # main_window.child.textEdit.append("Read Memory 0x%08x - 0x%08x Response Failed (NACK), %04d / %04d" % (
                                # addr, addr + 128 - 1, num + 1, num_128))
                                ...
                                # main_window.child.textEdit.append("Read Memory 0x%08x - 0x%08x Response Failed (Unknown), %04d / %04d" % (
                                # addr, addr + 128 - 1, num + 1, num_128))
                    ...
                    if time.time() >= prev_time + 1.0:
                        ...
                        main_window.child.textEdit.append("Read Memory 0x%08x - 0x%08x Response Failed (Timeout), %04d / %04d" % (
                        addr, addr + 128 - 1, num + 1, num_128))
                        break
            ...
            if check_get_checksum == True:
                main_window.child.textEdit.append("Got checksum (0x%08x - 0x%08x): 0x%08x, %d bytes" % (
                0x08000000, 0x08000000 + (num_128 * 128) - 1, crc_128_ack, (num_128 * 128)))
            else:
...
def verify_checksum(crc_128, crc_128_ack):
    main_window.child.count += 10
    main_window.child.progressBar.setValue(main_window.child.count)
    if crc_128 == crc_128_ack:
        main_window.child.textEdit.append("Checksum matched (.bin file: 0x%08x, Memory: 0x%08x)" % (crc_128, crc_128_ack))
        ...
        main_window.child.textEdit.append("Checksum not matched (.bin file: 0x%08x, Memory: 0x%08x)" % (crc_128, crc_128_ack))
